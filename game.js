...
const game = {
    ...
    attemptCrime: function(crime, chance, xp) {
        ...
        if (Math.random() > successChance) {
            ...
            const reward = this.crimeRewards[crime] || 0;
    ...
    attemptCarTheft: function(crime, chance, xp) {
        ...
        if (Math.random() > successChance) {
            ...
            const reward = this.carTheftRewards[crime] || 0;
    ...
    prisonCountdown: function() {
        const prisonTimer = document.getElementById("prison-timer");
        const prisonTimeSpan = document.getElementById("prison-time");
        
        if (this.prisonTime <= 0) {
            prisonTimer.style.display = "none";
            return;
        }
        
        prisonTimer.style.display = "block";
        prisonTimeSpan.textContent = `${this.prisonTime}s`;
    ...
    crimeCooldownTimer: function() {
        const crimeTimer = document.getElementById("crime-timer");
        const crimeCooldownSpan = document.getElementById("crime-cooldown");
        
        if (this.crimeCooldown <= 0) {
            crimeTimer.style.display = "none";
            return;
        }
        
        crimeTimer.style.display = "block";
        crimeCooldownSpan.textContent = `${this.crimeCooldown}s`;
    ...
    carTheftCooldownTimer: function() {
        const carTheftTimer = document.getElementById("car-theft-timer");
        const carTheftCooldownSpan = document.getElementById("car-theft-cooldown");
        
        if (this.carTheftCooldown <= 0) {
            carTheftTimer.style.display = "none";
            return;
        }
        
        carTheftTimer.style.display = "block";
        carTheftCooldownSpan.textContent = `${this.carTheftCooldown}s`;
    ...
    loadGame: function() {
        ...
        if (saveData) {
            try {
                ...
                // Si le jeu a été fermé, réduire les timers en fonction du temps écoulé
                if (data.lastSave) {
                    const timeElapsed = Math.floor((Date.now() - data.lastSave) / 1000);
                    this.prisonTime = Math.max(0, this.prisonTime - timeElapsed);
                    this.crimeCooldown = Math.max(0, this.crimeCooldown - timeElapsed);
                    this.carTheftCooldown = Math.max(0, this.carTheftCooldown - timeElapsed);
                }
            } catch (e) {
